...
El proyecto trae un script de seed (`database-seeder.service.ts`) que lee un archivo `load-seed.yaml` donde ustedes definen **cuántos registros crear por tabla y cómo se distribuyen** (tiendas, zonas, catálogos, productos, promociones, pedidos históricos).
