...
```bash
aws ec2 create-security-group  --group-name chiper-rds-sg  --description "Acceso a RDS de Chiper"  --vpc-id <VPC_ID>
```
...
aws rds modify-db-instance --db-instance-identifier chiper-rds --publicly-accessible --apply-immediately

# 3. Espere a que el cambio se aplique (puede tardar 1-2 minutos)
aws rds describe-db-instances --db-instance-identifier chiper-rds --query "DBInstances[0].PubliclyAccessible"
...
DB_HOST=<DB_HOST> DB_PORT=5432 DB_USER=postgres DB_PASSWORD=<PASSWORD_SEGURA> DB_NAME=chiper npm run db:seed
```

Al terminar, **revierta ambos cambios** para no dejar la base expuesta:

```bash
aws rds modify-db-instance --db-instance-identifier chiper-rds --no-publicly-accessible --apply-immediately
