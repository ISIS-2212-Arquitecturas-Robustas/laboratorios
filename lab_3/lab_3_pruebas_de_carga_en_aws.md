...
*Diagrama del despliegue objetivo: el ALB recibe el tráfico generado por JMeter (desde su computador) y lo distribuye entre las tres instancias EC2 de aplicación (`chiper-app-1/2/3`), las cuales se conectan por IP privada a la instancia EC2 de base de datos (`chiper-db`).*

> [!NOTE]
> El ícono usado para `chiper-db` es distinto al de las instancias de app **solo como convención visual** para diferenciar su rol (base de datos vs. aplicación). En términos de infraestructura, `chiper-db` **sigue siendo una instancia EC2** igual que las de app, solo que ejecuta un contenedor de PostgreSQL en lugar del monolito NestJS.
>
...
> | --- | --- | --- | --- | --- |
> | `chiper-db` | `t2.medium` | 2 | 4 GiB | 8 GB |
> | `chiper-app-1` | `t2.medium` | 2 | 4 GiB | 8 GB |
> | `chiper-app-2` | `t2.medium` | 2 | 4 GiB | 8 GB |
> | `chiper-app-3` | `t2.medium` | 2 | 4 GiB | 8 GB |
...
1. En la consola de EC2, seleccione la instancia `chiper-db`.
2. Presione **Connect**.
...
> - Para `chiper-db`: cuando la **instancia** fue detenida (stopped) y necesita reiniciar el contenedor de PostgreSQL.
...
1. Descargue el archivo `load-tests.jmx` (incluye las pruebas **GET** y **POST**). Este archivo corresponde al **repositorio del backend** (el mismo que clonó en la sección 4.4.3, rama `load_tests`); búsquelo en la raíz del proyecto o en la carpeta `test/` del repositorio clonado. Si no lo encuentra en su copia local, consulte con el personal docente el link directo de descarga.
...
3. **Security Groups** (`chiper-ssh`, `chiper-db`, `chiper-http`): solo pueden eliminarse una vez no estén asociados a ninguna instancia o ALB activo; en la consola de EC2 → **Security Groups**, selecciónelos y use **Actions → Delete security groups**.
